    ...
    <div class="goodtype" v-for="(item,index) in arrs" :key="index">
      ...
      <ul>
        <li v-for="item2 in item" :key="item2._id"    @click="godetails(item2.url,item2.brand,item2.title,item2.price.substr(0,7),item2.type)">
          ...
          <p v-text="item2.title.slice(4)"></p>
        </li>
      </ul>
    </div>
  </div>
...
export default {
  ...
  created() {
    //发送请求获取顶部的图片
    goodApi.getTypeListImg("香水").then(res => {
      console.log(res.data.data[0].url, 88888);
      this.imgurl = res.data.data[0].url;
    });

    //获取每个分类的小分类图片
    this.typelist.forEach(item => {
      goodApi.getSmallTypeListImg("xiangshui", 2, 5, item).then(res => {
